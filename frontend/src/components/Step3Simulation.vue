          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- 범용 폴백: 알 수 없는 타입 또는 위에서 처리되지 않은 content -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Step2에서 전달받은 최대 라운드 수
  minutesPerRound: {
    type: Number,
...
const resetAllState = () => {
  ...
  stopPolling()  // 이전에 실행 중인 폴링 중지
}

// 시뮬레이션 시작
const doStartSimulation = async () => {
  if (!props.simulationId) {
    addLog('오류: simulationId 가 없습니다')
    return
  }

  // 이전 시뮬레이션의 영향을 받지 않도록 모든 상태 초기화
  resetAllState()
  ...
  try {
    ...
    addLog('동적 그래프 업데이트 모드 활성화됨')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ 이전 시뮬레이션 로그 정리 완료, 새로 시작합니다')
      }
...
// 각 플랫폼의 이전 라운드 추적 (변화 감지 및 로그 출력용)
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 추가 확인: 백엔드가 runner_status를 아직 업데이트하지 않았지만 플랫폼이 완료를 보고한 경우
      // twitter_completed 및 reddit_completed 상태 확인
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ 모든 플랫폼 시뮬레이션 종료 감지됨')
        }
        addLog('✓ 시뮬레이션 완료')
        phase.value = 2
        stopPolling()
...
const checkPlatformsCompleted = (data) => {
  ...
  // 하나 이상의 플랫폼이 완료된 경우 활성화된 모든 플랫폼 완료 여부 확인
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // all_actions 를 사용하여 전체 액션 목록 가져오기
      ...
      // 자동 스크롤 없음, 사용자가 타임라인을 자유롭게 탐색 가능
      // 새 액션은 하단에 추가됨
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('보고서 생성 요청이 전송되었습니다. 잠시만 기다려 주세요...')
    return
  }
