    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - 완료 후 표시 -->
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>심층 인터랙션으로 이동</span>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (내용 생성 완료, 전체 섹션은 아직 미완성일 수 있음) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // DOM 업데이트 후 버튼 위치 유지를 위해 스크롤 위치 조정
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // 전구 아이콘 - 인사이트 표현
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // 지구본 아이콘 - 파노라마 검색 표현
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // 사용자 아이콘 - 대화 표현
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // 번개 아이콘 - 빠름 표현
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // 차트 아이콘 - 통계 표현
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // 데이터베이스 아이콘 - 엔티티 표현
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const factMatch = text.match(/관련 예측 사실:\s*(\d+)건/)
    const entityMatch = text.match(/관련 엔티티:\s*(\d+)개/)
    const relMatch = text.match(/관계 체인:\s*(\d+)건/)
    ...
    // 핵심 엔티티 추출 - 요약 및 관련 사실 수 포함 전체 추출
    ...
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const relatedMatch = block.match(/관련 사실:\s*(\d+)건/)
        return {
...
const parsePanorama = (text) => {
  ...
  try {
    // 쿼리 추출
    const queryMatch = text.match(/쿼리:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // 통계 데이터 추출
    const nodesMatch = text.match(/총 노드 수:\s*(\d+)/)
    const edgesMatch = text.match(/총 엣지 수:\s*(\d+)/)
    const activeMatch = text.match(/현재 유효 사실:\s*(\d+)건/)
    const histMatch = text.match(/이력\/만료 사실:\s*(\d+)건/)
    ...
    // 과거/만료된 사실 추출 - 전체 추출, 수량 제한 없음
    const histSection = text.match(/### 【이력\/만료 사실】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // 인터뷰 인원 수 추출 (예: "5 / 9 명의 시뮬레이션 Agent")
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // 형식1: 숫자. **이름（index=X）**：이유
        // 예: 1. **동문_345（index=1）**：무한대학교 동문으로서...
        ...
        // 형식2: - 선택 이름（index X）：이유
        // 예: - 선택_학부모_601（index 0）：학부모 집단 대표로서...
        if (!headerMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // 답변 추출 - Twitter와 Reddit으로 분리
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*핵심 인용문|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()

        // Twitter와 Reddit 답변 분리
        const twitterMatch = answerText.match(/【Twitter 플랫폼 답변】\n?([\s\S]*?)(?=【Reddit 플랫폼 답변】|$)/)
        const redditMatch = answerText.match(/【Reddit 플랫폼 답변】\n?([\s\S]*?)$/)
        ...
        // 플랫폼 폴백 로직 (구버전 형식 호환: 하나의 플랫폼 표시만 있는 경우)
        if (!twitterMatch && redditMatch) {
          // Reddit 답변만 있는 경우, 플레이스홀더 텍스트가 아닐 때만 기본 표시로 복사
          ...
          // 플랫폼 구분 표시 없음 (매우 구버전 형식), 전체를 답변으로 처리
          interview.twitterAnswer = answerText
        }
      }

      // 핵심 인용구 추출 (다양한 따옴표 형식 호환)
      const quotesMatch = block.match(/\*\*핵심 인용문:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // > "text" 형식 우선 매칭
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // 폴백: > "text" 또는 > \u201Ctext\u201D (중국어 따옴표) 매칭
        if (!quoteMatches) {
    ...
    // 인터뷰 요약 추출
    const summaryMatch = text.match(/### 인터뷰 요약 및 핵심 의견\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    // 검색 쿼리 추출
    const queryMatch = text.match(/검색 쿼리:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    // 결과 수 추출
    const countMatch = text.match(/(\d+)개의 관련 정보 발견/)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `관계 체인 (${props.result.relations.length})`)
        ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '시계열 기억에서 연관된 최신 핵심 사실'),
            h('span', { class: 'panel-count' }, `총 ${props.result.facts.length}건`)
          ]),
          ...
          }, expandedFacts.value ? `접기 ▲` : `전체 ${props.result.facts.length}건 펼치기 ▼`)
        ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          ...
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) => 
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount}건`)
              ])
            )
          ),
          ...
          }, expandedEntities.value ? `접기 ▲` : `전체 ${props.result.entities.length}개 펼치기 ▼`)
        ]),
        
        // Relations Tab
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '관계 체인'),
            h('span', { class: 'panel-count' }, `총 ${props.result.relations.length}건`)
          ]),
          ...
          }, expandedRelations.value ? `접기 ▲` : `전체 ${props.result.relations.length}건 펼치기 ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '드리프트 쿼리로 생성된 분석 하위 질문'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, '현재 핵심 기억 없음'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, '핵심 엔티티 없음'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, '관계 체인 없음')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `현재 유효 기억 (${props.result.activeFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '현재 유효 기억'),
            h('span', { class: 'panel-count' }, `총 ${props.result.activeFacts.length}건`)
          ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `총 ${props.result.historicalFacts.length}건`)
          ]),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '관련 엔티티 없음'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（해당 플랫폼에서 응답 없음）' || t === '(해당 플랫폼에서 응답 없음)' || t === '[응답 없음]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 두 가지 번호 형식 지원:
      // 1. "질문X：" 또는 "질문X:" (백엔드 신규 형식 - 한국어 질문 번호)
      // 2. "1. " 또는 "\n1. " (숫자+점, 구버전 형식 호환)
      let matches = []
      let match
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // 분할 실패: 첫 번째 질문은 전체 답변 반환, 나머지는 빈 문자열 반환
      return qIdx === 0 ? answer : ''
    }

    // 특정 질문에 이중 플랫폼 답변이 있는지 확인 (플레이스홀더 텍스트 필터링)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // 두 플랫폼 모두 실제 답변이 있고 (플레이스홀더 아님) 내용이 다른 경우
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // 이중 플랫폼 전환 버튼 (실제 이중 플랫폼 답변이 있을 때만 표시)
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '관련 결과 없음'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `접기 ▲` : `전체 ${props.result.facts.length}건 펼치기 ▼`)
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `총 ${props.result.edges.length}건`)
          ]),
...
const activeStep = computed(() => {
  ...
  // active가 없으면 마지막 done 단계 반환
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]

  // 그 외에는 첫 번째 단계 반환
  return steps[0] || { noLabel: '--', title: '시작 대기 중', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // 섹션 제목이 외부에 표시되므로 시작 부분의 2단계 제목(## xxx) 제거
  ...
  // 줄바꿈 처리 - 빈 줄은 단락 구분, 단일 줄바꿈은 <br>로 변환
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // <p><br>이 블록 요소 앞에 오는 경우 정리 (불필요한 빈 줄로 인해 발생)
  ...
  // 비연속 순서 목록 번호 수정: 단일 <ol>이 단락 내용으로 분리될 때 번호 연속 유지
...
const getLogLevelClass = (log) => {
  ...
  // INFO는 기본 색상 사용, success로 표시하지 않음
  return ''
}
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // 로딩 상태 확실히 초기화
            emit('update-status', 'completed')
            stopPolling()
            // 스크롤 로직은 반복 종료 후 nextTick에서 통합 처리
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // 작업 완료 시 맨 위로 스크롤; 그 외에는 최신 로그를 따라 아래로 스크롤
            if (isComplete.value) {
...
// 최종 답변 내용 추출 - LLM response에서 섹션 내용 추출
const extractFinalContent = (response) => {
  if (!response) return null

  // <final_answer> 태그 내의 내용 추출 시도
  ...
  // Final Answer: 이후 내용 찾기 시도 (다양한 형식 지원)
  // 형식1: Final Answer:\n\n내용
  // 형식2: Final Answer: 내용
  ...
  // 한국어 최종 답변 패턴: Final Answer로 통일되었으므로 해당 항목 제거 후 영문 패턴만 사용
  // (최종 답변 형식: Final Answer: ...)


  // ## 또는 # 또는 >로 시작하면 직접적인 markdown 내용일 가능성
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }

  // 내용이 길고 markdown 형식을 포함하는 경우, 사고 과정 제거 후 반환 시도
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Thought:로 시작하는 사고 과정 제거
